...
training_dataset = Mesh_Dataset3(data_list_path=train_list,
...
val_dataset = Mesh_Dataset3(data_list_path=val_list,
                           num_classes=num_classes,
                           #patch_size=200)
                           #patch_size=12000)
                           #patch_size=6000)
...
model = MBESegNet2(num_classes=num_classes, num_channels=num_channels)
#model = model.cuda()
...
print('best_val_dsc: ', best_val_dsc)
#cudnn
...
for epoch in range(num_epochs):
    ...
    for i_batch, batched_sample in enumerate(train_loader):
        ...
        #aug_loss_weights_np = np.array([0.3, 0.3, 0.3, 0.5])
        #aug_loss_weights_np = np.array([0.1, 0.1, 0.1, 0.5])
        #aug_loss_weights_np = np.array([0.1, 0.1, 0.3, 0.5])
        aug_loss_weights_np = np.array([1, 1, 1, 0.5])
        ...
        for i in range(num_layers):
            #temp = new_xyz[i]
            #a,b,c = temp
            #temp = temp.view(a,b,-1, n_pts)
            #print('new_xyz[i] size: ', new_xyz[i].size())
            #print('xyz[i] size: ', xyz[i].size())
            a, b, c = new_xyz[i].size()
            new_xyz[i] = new_xyz[i].view(a, b, -1, n_pts)
            #print('new_xyz[i] size after view: ', new_xyz[i].size())
            #print('new_xyz[i] size: ', new_xyz[i].size())
            centroids = torch.mean(new_xyz[i], dim=2)
            #print('centroids size: ', centroids.size())
            #print('xyz[i] size: ', xyz[i].size())
            #a, b, c = xyz[i].size()
            #xyz[i] = xyz[i].view(a, b, -1, n_pts)
            #xyz[i] = xyz[i].permute(0, 2, 1)
            #print('xyz[i] size after view: ', xyz[i].size())
            #centroids2 = torch.mean(xyz[i], dim=2)
            #print('centroids2 size: ', centroids2.size())
            #relative_dis = torch.sqrt(torch.sum(torch.square(centroids-xyz[i]), axis=1) + 1e-12)
            relative_dis = torch.sqrt(torch.sum(torch.square(centroids-xyz), axis=1) + 1e-12)
            #relative_dis = torch.sqrt(torch.sum(torch.square(centroids-centroids2), axis=1) + 1e-12)
        ...
        #print('aug_loss requires_grad: ', aug_loss.requires_grad)
        dice_loss = Generalized_Dice_Loss(outputs, one_hot_labels, class_weights)
        loss = dice_loss + aug_loss
