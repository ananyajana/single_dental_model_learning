...
num_channels = 24
#num_channels = 15
if network == 'meshsegnet' or network == 'mbesegnet':
    num_channels = 15
print(num_classes)
...
if network == 'pointnet':
    ...
    #model = DGCNN_semseg(num_classes=num_classes, num_channels=3, k=k, emb_dims=emb_dims, dropout=dropout)
...
elif network == 'pointmlp':
    model = pointMLP_seg(num_classes=num_classes, num_channels=num_channels)
elif network == 'randlanet':
    model = RandLANet(d_in=num_channels, num_classes=num_classes, num_neighbors=16, decimation=4, device=device)
elif network == 'curvenet':
    model = CurveNet(num_classes=num_classes)
elif network == 'pct':
    model = Point_Transformer_partseg(num_classes=num_classes, num_channels=3)
elif network == 'mbesegnet':
        ...
        for phase_str in ['test']:
            ...
            for idx in range(len(data_list)):
                ...
                if network == 'meshsegnet':
                    ...
                    X = X.reshape([1, X.shape[0], X.shape[1]])
                    inputs = torch.from_numpy(X).to(device, dtype=torch.float)
                    if network == 'mmnet' or network == 'mbesegnet':
                        outputs, _, _ = model(inputs)
                    else:
