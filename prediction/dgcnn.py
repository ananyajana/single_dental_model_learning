...
class DGCNN_semseg(nn.Module):
    ...
    def __init__(self, num_classes, num_channels, k, emb_dims, dropout):
        ...
        self.conv8 = nn.Sequential(nn.Conv1d(512, 256, kernel_size=1, bias=False ),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        # adding one more layer to make it similar to pointnet
        ...
        self.conv10 = nn.Conv1d(128, self.num_classes, kernel_size=1, bias=False )
    ...
    def forward(self, x):
        ...
        x = self.dp1(x)
        x = self.conv10(x)                       # (batch_size, 256, num_points) -> (batch_size, 13, num_points)
        #x = x.transpose(2, 1)
        x = x.transpose(2, 1).contiguous()
        #x = torch.nn.Softmax(dim=-1)(x.view(-1,self.k))
        #x = torch.nn.LogSoftmax(dim=-1)(x.view(-1,self.k))
        x = torch.nn.LogSoftmax(dim=-1)(x.view(-1,self.num_classes))
